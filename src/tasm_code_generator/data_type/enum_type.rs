...
impl EnumType {
    ...
    pub(crate) fn load_from_memory(&self, state: &mut CompilerState) -> Vec<LabelledInstruction> {
        let (pointer_pointer, get_variant_data_pointers, subroutines) =
            self.get_variant_data_pointers(state);
        ...
        // We have all pointers now. Now we should loop over them. But how do we
        // know what type each field has? We don't! We probably need to
        // *keep* the discriminant around and then write the code to copy a field
        // from memory to stack based on the knowledge of the types.
    ...
    fn get_variant_data_pointers(
        ...
        // Allocate memory for all results, plus one word for length indication.
        let pointer_for_result: u64 = state
            .global_compiler_state
            .snippet_state
            .kmalloc(max_field_count + 1)
            .try_into()
            .unwrap();
            ...
            self.variants.iter().enumerate()
        {
            ...
            acc_code.append(&mut triton_asm!(
                ...
                eq
                // *discriminant (discriminant == haystack)
            ...
            let subroutine = match data_fields.element_count() {
                0 => triton_asm!(
                    {subroutine_label}:
                        ...
                        push {pointer_for_result}

                        push 0
                        // _ *discriminant **fields field_count

                        write_mem
                        // _ *discriminant **fields
                ...
                1 => {
                    ...
                    let get_first_field = match static_encoding_length {
                        Some(_static_size) => triton_asm!(
                            // _ *discriminant
                            push 1
                            add
                            // _ *first_field
                    ...
                    triton_asm!(
                        ...
                        push 1
                        add
                        // _ *discriminant *fields[0]

                        dup 1
                        {&get_first_field}
                        // _ *discriminant *fields[0] *first_field
        ...
        (
            pointer_for_result.into(),
            acc_code.try_into().unwrap(),
            subroutines,
        )
    }
