...
fn main() {
    ...
    let omega: BFieldElement = tasm::tasm_io_read_stdin_bfe();
    ...
    while i < size {
        tasm::tasm_io_write_to_stdout_xfe(output[i]);
        i += 1;
    }
...
mod tests {
    ...
    fn xfe_ntt_test() {
        ...
        for input_length in [2, 4, 8, 16, 32, 64] {
            ...
            let (rust_ast, _, _) = ozk_parsing::parse_main_and_structs("recufier", "xfe_ntt");
